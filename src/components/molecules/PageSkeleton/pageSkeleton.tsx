...
type PageSkeletonVariant = "models" | "profiles" | "backups" | "usageStats" | "diagnostics" | "settings";
...
export function PageSkeleton({ variant, label }: { variant: PageSkeletonVariant; label: string }) {
  return (
    <div role="status" aria-live="polite" aria-label={label} className="space-y-4">
      ...
      {variant === "settings" && <>
        <Block className="h-16" />
        <div className="grid gap-4 lg:grid-cols-2">{[1, 2].map((item) => <Block key={item} className="h-72" />)}</div>
      </>}
