...
describe("PageSkeleton", () => {
  ...
  it("renders a structured, accessible status for the settings variant", () => {
    render(<PageSkeleton variant="settings" label="Loading configuration…" />);

    expect(screen.getByRole("status", { name: "Loading configuration…" })).not.toBeNull();
    const blocks = document.querySelectorAll('[data-slot="skeleton"]');
