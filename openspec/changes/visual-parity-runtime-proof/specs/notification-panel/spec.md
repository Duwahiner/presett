...
In visual audit mode, notification content MUST be deterministic and read-only.
(Previously: the panel spec did not constrain audit-mode behavior.)
...
#### Scenario: Audit mode blocks notification actions
- GIVEN visual audit mode is on
- WHEN the user attempts to dismiss or act on a notification
- THEN the mutation is denied
- AND the panel contents stay unchanged
